...
def esearch(retstart, retmax):      
    ...
    esearch_url = urlstr + dbstr + ret_range_str + retmodestr + querystr + andboolean + pubdate
    try:
        ...
        #print(f"{json.dumps(es_data)}")
        #import info from esearch return is: idlist & count;
        # if result count < specified retmax, it will return retmax= count
        # return from pubmed is like:
        #{"header": {"type": "esearch", "version": "0.3"}, 
        # "esearchresult": {"count": "5", "retmax": "5", "retstart": "0", 
        # "idlist": ["34436303", "33831101", "30623481", "29156601", "23733102"], 
        # "translationset": [{"from": "chlorella", "to": "\"chlorella\"[MeSH Terms] 
        # OR \"chlorella\"[All Fields] OR \"chlorellas\"[All Fields] 
        # OR \"chlorellae\"[All Fields]"}, {"from": "skin care", 
        # "to": "\"skin care\"[MeSH Terms] OR (\"skin\"[All Fields] 
        # AND \"care\"[All Fields]) OR \"skin care\"[All Fields]"}],
        # "querytranslation": "(\"chlorella\"[MeSH Terms] OR \"chlorella\"[All Fields] 
        # OR \"chlorellas\"[All Fields] OR \"chlorellae\"[All Fields]) 
        # AND (\"skin care\"[MeSH Terms] OR (\"skin\"[All Fields] 
        # AND \"care\"[All Fields]) OR \"skin care\"[All Fields]) 
        # AND 2000/01/01:2025/12/31[Date - Publication]"}}
